	...
	**/
	...
	ini_set(
		...
		get_include_path().PATH_SEPARATOR
			...
			.PATH_CONTROLLERS.'admin'.DIRECTORY_SEPARATOR.PATH_SEPARATOR
	);
	...
	try {
		...
		if (
			...
			&& (
				...
				|| is_readable(PATH_CONTROLLERS.'admin'.DIRECTORY_SEPARATOR.$_GET['area'].EXT_CLASS)
			)
		) {
		...
		elseif (is_string($view)) {
			...
			elseif (strpos($view, 'redirect:') !== false) {
				...
				$view = new RedirectView(PATH_ADMIN.'?area='.$area);
			}
		}
		...
		if (!$view instanceof RedirectView) {
			$model->
				set('selfUrl', PATH_ADMIN.'?area='.$controllerName)->
				set('baseUrl', PATH_ADMIN)->
