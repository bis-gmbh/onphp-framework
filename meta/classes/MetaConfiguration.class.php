	...
	final class MetaConfiguration extends Singleton implements Instantiatable
	{
				...
				if (
					...
					if ($info->implementsInterface('\Onphp\Prototyped'))
						$this->checkClassSanity(
							$class,
							new \ReflectionClass($class->getFullClassName('Proto'))
						);
					
					if ($info->implementsInterface('\Onphp\DAOConnected'))
						$this->checkClassSanity(
							$class,
							new \ReflectionClass($class->getFullClassName('', 'DAO'))
						);
					...
					if (
						...
						$fullName = $class->getFullClassName();
						$object = new $fullName(call_user_func(array($fullName, 'getAnyId')));
					...
					foreach ($class->getProperties() as $propertyName => $property) {
						Assert::isTrue(
							$property->toLightProperty($class)
								== $proto->getPropertyByName($propertyName),
							
							'defined property does not match autogenerated one - '
								.$class->getFullClassName().'::'.$property->getName()
						);
					}
		...
		**/
		private function guessPattern($name)
		{
			...
			if (is_readable(ONPHP_META_PATTERNS.$name.'Pattern'.EXT_CLASS))
